...
static constexpr void escp_5(string_view in, string* out)
{
...
static constexpr void escp_6(string_view in, string* out)
{
  ranges::for_each(in,[&](char i) {
    if (i == '%') {
      out->append("%p");
...
string hobject_t::to_str_esc5() const
{
  string out;
...
static /*constexpr*/ std::string escape_special_chars(std::string_view in)
{
  std::string out;
  out.reserve(in.size() * 2);

  using c2_t = std::pair<char, char>;
  //   static const c2_t c2[] = {
  //         { '%', 'p' },
  //         { '.', 'e' },
  //         { '_', 'u' }
  //         };
...
static void escp_6(const string& in, string* out)
{
  for (auto i : in) {
    if (i == '%') {
      out->push_back('%');
...
string hobject_t::to_str2() const
{
  ...
  if (snap == CEPH_NOSNAP) {
    return fmt::format(FMT_COMPILE("{:016X}.{:08X}.head.{}.{}.{}"), poolid, revhash,
		       escape_special_chars(oid.name), escape_special_chars(get_key()),
		       escape_special_chars(nspace));

  } else if (snap == CEPH_SNAPDIR) {
    return fmt::format(FMT_COMPILE("{:016X}.{:08X}.snapdir.{}.{}.{}"), poolid, revhash,
		       escape_special_chars(oid.name), escape_special_chars(get_key()),
		       escape_special_chars(nspace));
  } else {

    return fmt::format(FMT_COMPILE("{:016X}.{:08X}.{:X}.{}.{}.{}"), poolid, revhash, (unsigned long long)snap,
		       escape_special_chars(oid.name), escape_special_chars(get_key()),
		       escape_special_chars(nspace));
  }

  //   return fmt::format("{:16X}.{:8X}.{}.{}.{}.{}", poolid, revhash, snap_data,
  // escape_special_chars(oid.name), escape_special_chars(get_key()),
  // escape_special_chars(nspace));
  //
  //   out.append(snap_with_hash, t);
  //
  //   out.push_back('.');
  //   out += escape_special_chars(oid.name);
  //
  //
  //
  // //   append_escaped(oid.name, &out);
...
string hobject_t::to_str4() const
{
  uint64_t poolid(pool);
  uint32_t revhash(get_nibblewise_key_u32());
...
string hobject_t::to_str4() const
{
  ...
  if (snap == CEPH_NOSNAP) {
    ...
    out = fmt::format(FMT_COMPILE("{:016X}.{:08X}.{:X}."), poolid, revhash, (unsigned long long)snap);
  }
...
void hobject_t::generate_test_instances(list<hobject_t*>& o)
{
  ...
  o.push_back(
    new hobject_t(object_t("oname2"), string("okey"), CEPH_NOSNAP, 67, 0, "n1"));
  o.push_back(
    new hobject_t(object_t("oname3"), string("oname3"), CEPH_SNAPDIR, 910, 1, "n2"));
}
...
list<ghobject_t*> ghobject_t::generate_test_instances()
{
  ...
  o.push_back(new ghobject_t(
    hobject_t(object_t("oname2"), string("okey"), CEPH_NOSNAP, 67, 0, "n1"), 1,
    shard_id_t(0)));
  o.push_back(new ghobject_t(
    hobject_t(object_t("oname2"), string("okey"), CEPH_NOSNAP, 67, 0, "n1"), 1,
    shard_id_t(1)));
  o.push_back(new ghobject_t(
    ...
    shard_id_t(2)));
  o.push_back(new ghobject_t(
    hobject_t(object_t("oname3"), string("oname3"), CEPH_SNAPDIR, 910, 1, "n2"), 1,
    shard_id_t(0)));
  o.push_back(new ghobject_t(
    hobject_t(object_t("oname3"), string("oname3"), CEPH_SNAPDIR, 910, 1, "n2"), 2,
    shard_id_t(0)));
  o.push_back(new ghobject_t(
    hobject_t(object_t("oname3"), string("oname3"), CEPH_SNAPDIR, 910, 1, "n2"), 3,
    shard_id_t(0)));
  o.push_back(new ghobject_t(
    hobject_t(object_t("oname3"), string("oname3"), CEPH_SNAPDIR, 910, 1, "n2"), 3,
    shard_id_t(1)));
